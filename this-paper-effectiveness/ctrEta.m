function [tao, d] = ctrEta(index, Omega, L, Eta, Etad, V, Varphi1, loop)
    ...
    for j=neighbor_node
        ...
        posdd = posdd + Omega(index, j)*Eta{j}(loop,1:2);
        yawdd = yawdd + -1*(Eta{j}(loop,3));
    end
    K = diag([60,60,60]); % 滑模面线性系数, 也是线性系统的A
%     K = diag([100, 100, 8]); h = 100;
    z = [epos, eyaw]; zd = [eposd, eyawd]; zdd = [posdd, yawdd];
    s = zd + z*K;
    
    %% 黎卡提方程
    Gamma = diag([5, 5, 5]); % 误差权衡系数
    Lambda = diag([100, 100, 100]); % 控制力矩权衡系数
    B = diag([1, 1, 1]); % 线性系统的B
